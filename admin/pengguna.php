...
// Kode Php
include '../php/admin/pengguna.php';
...
<body class="h-screen flex flex-col bg-gray-50">
    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <div class="flex-1 flex flex-col bg-gray-50 p-4 overflow-y-auto">
            ...
            <!-- Title -->
            <h2 class="text-2xl font-semibold mb-6 text-gray-700">Tabel Pengguna</h2>

            <!-- Button Tambah Data -->
            <div class="mb-4">
                <a href="tambah_pengguna.php"
                    class="bg-[#ffb4a2] text-white px-4 py-2 rounded-md shadow-md hover:bg-red-400 transition">
            ...
            <div class="overflow-x-auto bg-white p-4 rounded-lg shadow-md">
                <table class="w-full border-collapse border border-gray-200">
                    <thead>
                        <tr class="bg-[#ffb4a2] text-white">
                            <th class="py-2 px-4 border border-gray-200">ID</th>
                            <th class="py-2 px-4 border border-gray-200">Nama</th>
                            <th class="py-2 px-4 border border-gray-200">Email</th>
                            <th class="py-2 px-4 border border-gray-200">Role</th>
                            <th class="py-2 px-4 border border-gray-200">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php while ($row = $resultPengguna->fetch_assoc()) { ?>
                        <tr class="hover:bg-gray-100">
                            <td class="py-2 px-4 border border-gray-200 text-center"><?php echo $row['id_pengguna']; ?></td>
                            <td class="py-2 px-4 border border-gray-200"><?php echo htmlspecialchars($row['nama']); ?></td>
                            <td class="py-2 px-4 border border-gray-200"><?php echo htmlspecialchars($row['email']); ?></td>
                            <td class="py-2 px-4 border border-gray-200 text-center">
                                <span class="px-3 py-1 rounded-full text-white 
                                <?php echo $row['role'] === 'admin' ? 'bg-green-500' : 'bg-blue-500'; ?>">
                                    <?php echo ucfirst($row['role']); ?>
                                </span>
                            </td>
                            <td class="py-2 px-4 border border-gray-200 text-center">
                                <a href="edit_pengguna.php?id=<?php echo $row['id_pengguna']; ?>" class="text-blue-500 mx-2">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="../php/admin/hapus_pengguna.php?id=<?php echo $row['id_pengguna']; ?>"
                                    class="text-red-500 mx-2" onclick="return confirm('Hapus pengguna ini?')">
            ...
            <div class="mt-6 text-center text-white text-sm w-full bg-[#FFB4A2] p-3 rounded-md">
                &copy; <?php echo date("Y"); ?> Listify. All Rights Reserved.
            </div>
